...
![netty-sample-08](../../source/images/ch-06/old/netty-sample-08.png)
...
- 初始分配较慢
- 没有JVM直接帮助管理内存，容易发生内存溢出。为了避免一直没有FULL GC，最终导致直接内存把物理内存被耗完。我们可以指定直接内存的最大值，通过 `-XX:MaxDirectMemorySize` 来指定，当达到阈值的时候，调用 `system.gc` 来进行一次FULL GC，间接把那些没有被使用的直接内存回收掉。